...
DEFAULT_FILLERS = {
    "eh", "emm", "em", "mmm", "este", "esto", "bueno", "digamos", "osea", "o sea",
}
...
def ensure_ffmpeg() -> tuple[str, str]:
    ...
    if not ffmpeg or not ffprobe:
        raise RuntimeError(
            "FFmpeg no está instalado o no está en PATH. Ejecuta install_windows.bat y vuelve a abrir la app."
        )
...
def detect_silences(path: Path, mode: str = "Natural") -> list[Cut]:
    ...
    proc = _run([
        ...
        "-af", f"silencedetect=noise={cfg['silence_db']}dB:d={cfg['min_silence']}",
        "-f", "null", "-"
    ])
    text = proc.stderr
    ...
    for idx, start in enumerate(starts):
        ...
        # Keep a little natural breathing room around speech.
...
def detect_fillers(words: list[dict], fillers: Iterable[str] | None = None) -> list[Cut]:
    ...
    cuts: list[Cut] = []
    for word in words:
        norm = _normalize_token(word["text"])
        if norm in filler_set:
            start = max(0.0, word["start"] - 0.03)
            end = word["end"] + 0.03
            cuts.append(Cut(start, end, "muletilla", f"Muletilla: {word['text']}"))
    return cuts


def detect_repetitions(words: list[dict]) -> list[Cut]:
    # Conservative MVP: catches immediately repeated words and repeated 2-word phrases.
    cuts: list[Cut] = []
